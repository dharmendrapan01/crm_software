...
class SiteVisitDone extends StatefulWidget {
  const SiteVisitDone({Key? key}) : super(key: key);

  @override
  State<SiteVisitDone> createState() => _SiteVisitDoneState();
}

class _SiteVisitDoneState extends State<SiteVisitDone> {
  List<Enqlist> result = [];
  ...
  //       Uri.parse(
  //           '$apiRootUrl/sitevisit_done.php?user_id=$userId&page_no=$page'),
  //       headers: headersData);
  ...
  Future getEnqData(userToken, userId, paraPage) async {
    ...
    var apiUrl = '$apiRootUrl/sitevisit_done.php';
  ...
  void addFavorite(leadId, userToken) async {
    ...
    Navigator.of(context).pushReplacement(MaterialPageRoute(builder: (context) => SiteVisitPage(tabIndex: 1)));
  }
