...
class FreshDeletedLead extends StatefulWidget {
  const FreshDeletedLead({Key? key}) : super(key: key);

  @override
  State<FreshDeletedLead> createState() => _FreshDeletedLeadState();
}

class _FreshDeletedLeadState extends State<FreshDeletedLead> {
  List<Enqlist> result = [];
  ...
  //       Uri.parse(
  //           '$apiRootUrl/freshdeleted_lead.php?user_id=$userId&page_no=$page'),
  //       headers: headersData);
  ...
  Future getEnqData(userToken, userId, paraPage) async {
    ...
    var apiUrl = '$apiRootUrl/freshdeleted_lead.php';
  ...
  void addFavorite(leadId, userToken) async {
    ...
    Navigator.of(context).pushReplacement(MaterialPageRoute(builder: (context) => FreshEnquiryPage(tabIndex: 3)));
  }
