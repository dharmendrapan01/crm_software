...
class QualifiedLead extends StatefulWidget {
  const QualifiedLead({Key? key}) : super(key: key);

  @override
  State<QualifiedLead> createState() => _QualifiedLeadState();
}

class _QualifiedLeadState extends State<QualifiedLead> {
  List<Enqlist> result = [];
  ...
  //       Uri.parse(
  //           '$apiRootUrl/allqualify_lead.php?user_id=$userId&page_no=$page'),
  //       headers: headersData);
  ...
  Future getEnqData(userToken, userId, paraPage) async {
    ...
    var apiUrl = '$apiRootUrl/allqualify_lead.php';
  ...
  void addFavorite(leadId, userToken) async {
    ...
    Navigator.of(context).pushReplacement(MaterialPageRoute(builder: (context) => AllenquiryLead(tabIndex: 3)));
  }
