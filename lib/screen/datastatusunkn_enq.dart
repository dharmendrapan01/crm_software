...
class DataStatusUnknEnq extends StatefulWidget {
  const DataStatusUnknEnq({Key? key}) : super(key: key);

  @override
  State<DataStatusUnknEnq> createState() => _DataStatusUnknEnqState();
}

class _DataStatusUnknEnqState extends State<DataStatusUnknEnq> {
  List<Enqlist> result = [];
  ...
  //       Uri.parse(
  //           '$apiRootUrl/datastatusunkn_enq.php?user_id=$userId&page_no=$page'),
  //       headers: headersData);
  ...
  Future getEnqData(userToken, userId, paraPage) async {
    ...
    var apiUrl = '$apiRootUrl/datastatusunkn_enq.php';
  ...
  void addFavorite(leadId, userToken) async {
    ...
    Navigator.of(context).pushReplacement(MaterialPageRoute(builder: (context) => DataEnquiryPage(tabIndex: 1)));
  }
