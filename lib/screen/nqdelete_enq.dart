...
import 'package:crm_software/nqdelete_page.dart';
...
class NotQualifiedDeleteEnq extends StatefulWidget {
  const NotQualifiedDeleteEnq({Key? key}) : super(key: key);

  @override
  State<NotQualifiedDeleteEnq> createState() => _NotQualifiedDeleteEnqState();
}

class _NotQualifiedDeleteEnqState extends State<NotQualifiedDeleteEnq> {
  List<Enqlist> result = [];
  ...
  //       Uri.parse(
  //           '$apiRootUrl/notqualified_delete.php?user_id=$userId&page_no=$page'),
  //       headers: headersData);
  ...
  Future getEnqData(userToken, userId, paraPage) async {
    ...
    var apiUrl = '$apiRootUrl/notqualified_delete.php';
  ...
  void addFavorite(leadId, userToken) async {
    ...
    Navigator.of(context).pushReplacement(MaterialPageRoute(builder: (context) => NotQualDeletePage()));
  }
