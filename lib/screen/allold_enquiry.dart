...
class AllOldEnquiry extends StatefulWidget {
  const AllOldEnquiry({Key? key}) : super(key: key);

  @override
  State<AllOldEnquiry> createState() => _AllOldEnquiryState();
}

class _AllOldEnquiryState extends State<AllOldEnquiry> {
  List<Enqlist> result = [];
  ...
  void getEnqData(userToken, userId, paraPage) async {
    ...
    var response = await http.get(
        Uri.parse(
            '$apiRootUrl/allold_enq.php?user_id=$userId&page_no=$page'),
        headers: headersData);
  ...
  void addFavorite(leadId, userToken) async {
    ...
    Navigator.of(context).pushReplacement(MaterialPageRoute(builder: (context) => OldEnquiryPage(tabIndex: 4)));
  }
