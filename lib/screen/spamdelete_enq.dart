...
class SpamDeleteEnquiry extends StatefulWidget {
  const SpamDeleteEnquiry({Key? key}) : super(key: key);

  @override
  State<SpamDeleteEnquiry> createState() => _SpamDeleteEnquiryState();
}

class _SpamDeleteEnquiryState extends State<SpamDeleteEnquiry> {
  List<Enqlist> result = [];
  ...
  //       Uri.parse(
  //           '$apiRootUrl/spamdelete_enq.php?user_id=$userId&page_no=$page'),
  //       headers: headersData);
  ...
  Future getEnqData(userToken, userId, paraPage) async {
    ...
    var apiUrl = '$apiRootUrl/spamdelete_enq.php';
  ...
  void addFavorite(leadId, userToken) async {
    ...
    Navigator.of(context).pushReplacement(MaterialPageRoute(builder: (context) => AllDeletedPage(tabIndex: 3)));
  }
