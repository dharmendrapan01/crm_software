...
import 'package:crm_software/qualified_page.dart';
...
class QualifiedDataLead extends StatefulWidget {
  const QualifiedDataLead({Key? key}) : super(key: key);

  @override
  State<QualifiedDataLead> createState() => _QualifiedDataLeadState();
}

class _QualifiedDataLeadState extends State<QualifiedDataLead> {
  List<Enqlist> result = [];
  ...
  //       Uri.parse(
  //           '$apiRootUrl/qualified_data.php?user_id=$userId&page_no=$page'),
  //       headers: headersData);
  ...
  Future getEnqData(userToken, userId, paraPage) async {
    ...
    var apiUrl = '$apiRootUrl/qualified_data.php';
  ...
  void addFavorite(leadId, userToken) async {
    ...
    Navigator.of(context).pushReplacement(MaterialPageRoute(builder: (context) => QualifiedPage(tabIndex: 4)));
  }
