...
class OldDeletedEnq extends StatefulWidget {
  const OldDeletedEnq({Key? key}) : super(key: key);

  @override
  State<OldDeletedEnq> createState() => _OldDeletedEnqState();
}

class _OldDeletedEnqState extends State<OldDeletedEnq> {
  List<Enqlist> result = [];
  ...
  void getEnqData(userToken, userId, paraPage) async {
    ...
    var response = await http.get(
        Uri.parse(
            '$apiRootUrl/olddeleted_enq.php?user_id=$userId&page_no=$page'),
        headers: headersData);
  ...
  void addFavorite(leadId, userToken) async {
    ...
    Navigator.of(context).pushReplacement(MaterialPageRoute(builder: (context) => OldEnquiryPage(tabIndex: 3)));
  }
