...
import 'package:crm_software/sitevisit_page.dart';
...
class SiteVisitDoneFoure extends StatefulWidget {
  const SiteVisitDoneFoure({Key? key}) : super(key: key);

  @override
  State<SiteVisitDoneFoure> createState() => _SiteVisitDoneFoureState();
}

class _SiteVisitDoneFoureState extends State<SiteVisitDoneFoure> {
  List<Enqlist> result = [];
  ...
  //       Uri.parse(
  //           '$apiRootUrl/svdone_foure.php?user_id=$userId&page_no=$page'),
  //       headers: headersData);
  ...
  Future getEnqData(userToken, userId, paraPage) async {
    ...
    var apiUrl = '$apiRootUrl/svdone_foure.php';
  ...
  void addFavorite(leadId, userToken) async {
    ...
    Navigator.of(context).pushReplacement(MaterialPageRoute(builder: (context) => SiteVisitPage(tabIndex: 5)));
  }
