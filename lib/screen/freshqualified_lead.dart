...
class FreshQualifiedLead extends StatefulWidget {
  const FreshQualifiedLead({Key? key}) : super(key: key);

  @override
  State<FreshQualifiedLead> createState() => _FreshQualifiedLeadState();
}

class _FreshQualifiedLeadState extends State<FreshQualifiedLead> {
  List<Enqlist> result = [];
  ...
  //       Uri.parse(
  //           '$apiRootUrl/freshqualified_lead.php?user_id=$userId&page_no=$page'),
  //       headers: headersData);
  ...
  Future getEnqData(userToken, userId, paraPage) async {
    ...
    var apiUrl = '$apiRootUrl/freshqualified_lead.php';
  ...
  void addFavorite(leadId, userToken) async {
    ...
    Navigator.of(context).pushReplacement(MaterialPageRoute(builder: (context) => FreshEnquiryPage(tabIndex: 2)));
  }
