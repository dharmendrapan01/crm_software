...
class SiteVisitDoneTwo extends StatefulWidget {
  const SiteVisitDoneTwo({Key? key}) : super(key: key);

  @override
  State<SiteVisitDoneTwo> createState() => _SiteVisitDoneTwoState();
}

class _SiteVisitDoneTwoState extends State<SiteVisitDoneTwo> {
  List<Enqlist> result = [];
  ...
  //       Uri.parse(
  //           '$apiRootUrl/svdone_two.php?user_id=$userId&page_no=$page'),
  //       headers: headersData);
  ...
  Future getEnqData(userToken, userId, paraPage) async {
    ...
    var apiUrl = '$apiRootUrl/svdone_two.php';
  ...
  void addFavorite(leadId, userToken) async {
    ...
    Navigator.of(context).pushReplacement(MaterialPageRoute(builder: (context) => SiteVisitPage(tabIndex: 3)));
  }
