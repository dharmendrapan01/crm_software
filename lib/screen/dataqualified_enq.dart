...
class DataQualifiedLead extends StatefulWidget {
  const DataQualifiedLead({Key? key}) : super(key: key);

  @override
  State<DataQualifiedLead> createState() => _DataQualifiedLeadState();
}

class _DataQualifiedLeadState extends State<DataQualifiedLead> {
  List<Enqlist> result = [];
  ...
  //       Uri.parse(
  //           '$apiRootUrl/dataqualify_enq.php?user_id=$userId&page_no=$page'),
  //       headers: headersData);
  ...
  Future getEnqData(userToken, userId, paraPage) async {
    ...
    var apiUrl = '$apiRootUrl/dataqualify_enq.php';
  ...
  void addFavorite(leadId, userToken) async {
    ...
    Navigator.of(context).pushReplacement(MaterialPageRoute(builder: (context) => DataEnquiryPage(tabIndex: 2)));
  }
