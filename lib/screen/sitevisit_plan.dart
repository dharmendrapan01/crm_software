...
class SiteVisitPlan extends StatefulWidget {
  const SiteVisitPlan({Key? key}) : super(key: key);

  @override
  State<SiteVisitPlan> createState() => _SiteVisitPlanState();
}

class _SiteVisitPlanState extends State<SiteVisitPlan> {
  List<Enqlist> result = [];
  ...
  //       Uri.parse(
  //           '$apiRootUrl/plan_sitevisit.php?user_id=$userId&page_no=$page'),
  //       headers: headersData);
  ...
  Future getEnqData(userToken, userId, paraPage) async {
    ...
    var apiUrl = '$apiRootUrl/plan_sitevisit.php';
  ...
  void addFavorite(leadId, userToken) async {
    ...
    Navigator.of(context).pushReplacement(MaterialPageRoute(builder: (context) => SiteVisitPage(tabIndex: 0)));
  }
