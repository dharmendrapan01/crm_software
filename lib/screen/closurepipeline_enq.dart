...
import 'package:crm_software/closerpipeline_page.dart';
...
class ClosurePipelineEnq extends StatefulWidget {
  const ClosurePipelineEnq({Key? key}) : super(key: key);

  @override
  State<ClosurePipelineEnq> createState() => _ClosurePipelineEnqState();
}

class _ClosurePipelineEnqState extends State<ClosurePipelineEnq> {
  List<Enqlist> result = [];
  ...
  //       Uri.parse(
  //           '$apiRootUrl/closerpipline_lead.php?user_id=$userId&page_no=$page'),
  //       headers: headersData);
  ...
  Future getEnqData(userToken, userId, paraPage) async {
    ...
    var apiUrl = '$apiRootUrl/closerpipline_lead.php';
  ...
  void addFavorite(leadId, userToken) async {
    ...
    Navigator.of(context).pushReplacement(MaterialPageRoute(builder: (context) => CloserPipeLinePage()));
  }
