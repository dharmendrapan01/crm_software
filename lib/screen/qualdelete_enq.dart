...
import 'package:crm_software/qualified_delete_page.dart';
...
class QualifiedDeleteEnq extends StatefulWidget {
  const QualifiedDeleteEnq({Key? key}) : super(key: key);

  @override
  State<QualifiedDeleteEnq> createState() => _QualifiedDeleteEnqState();
}

class _QualifiedDeleteEnqState extends State<QualifiedDeleteEnq> {
  List<Enqlist> result = [];
  ...
  //       Uri.parse(
  //           '$apiRootUrl/qualified_delete.php?user_id=$userId&page_no=$page'),
  //       headers: headersData);
  ...
  Future getEnqData(userToken, userId, paraPage) async {
    ...
    var apiUrl = '$apiRootUrl/qualified_delete.php';
  ...
  void addFavorite(leadId, userToken) async {
    ...
    Navigator.of(context).pushReplacement(MaterialPageRoute(builder: (context) => QualifiedDeletePage()));
  }
