...
class AllFreshEnquiry extends StatefulWidget {
  const AllFreshEnquiry({Key? key}) : super(key: key);

  @override
  State<AllFreshEnquiry> createState() => _AllFreshEnquiryState();
}

class _AllFreshEnquiryState extends State<AllFreshEnquiry> {
  List<Enqlist> result = [];
  ...
  //       Uri.parse(
  //           '$apiRootUrl/allfresh_enquiry.php?user_id=$userId&page_no=$page'),
  //       headers: headersData);
  ...
  Future getEnqData(userToken, userId, paraPage) async {
    ...
    var apiUrl = '$apiRootUrl/allfresh_enquiry.php';
  ...
  void addFavorite(leadId, userToken) async {
    ...
    Navigator.of(context).pushReplacement(MaterialPageRoute(builder: (context) => FreshEnquiryPage(tabIndex: 4)));
  }
