...
class AllQualifiedLead extends StatefulWidget {
  const AllQualifiedLead({Key? key}) : super(key: key);

  @override
  State<AllQualifiedLead> createState() => _AllQualifiedLeadState();
}

class _AllQualifiedLeadState extends State<AllQualifiedLead> {
  List<Enqlist> result = [];
  ...
  //       Uri.parse(
  //           '$apiRootUrl/allqualified_enq.php?user_id=$userId&page_no=$page'),
  //       headers: headersData);
  ...
  Future getEnqData(userToken, userId, paraPage) async {
    ...
    var apiUrl = '$apiRootUrl/allqualified_enq.php';
  ...
  void addFavorite(leadId, userToken) async {
    ...
    Navigator.of(context).pushReplacement(MaterialPageRoute(builder: (context) => QualifiedPage(tabIndex: 0)));
  }
