...
class QualifiedOld extends StatefulWidget {
  const QualifiedOld({Key? key}) : super(key: key);

  @override
  State<QualifiedOld> createState() => _QualifiedOldState();
}

class _QualifiedOldState extends State<QualifiedOld> {
  List<Enqlist> result = [];
  ...
  //       Uri.parse(
  //           '$apiRootUrl/oldqualify_enq.php?user_id=$userId&page_no=$page'),
  //       headers: headersData);
  ...
  Future getEnqData(userToken, userId, paraPage) async {
    ...
    var apiUrl = '$apiRootUrl/oldqualify_enq.php';
  ...
  void addFavorite(leadId, userToken) async {
    ...
    Navigator.of(context).pushReplacement(MaterialPageRoute(builder: (context) => QualifiedPage(tabIndex: 5)));
  }
