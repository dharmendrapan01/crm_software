...
class OldNotModifyEnq extends StatefulWidget {
  const OldNotModifyEnq({Key? key}) : super(key: key);

  @override
  State<OldNotModifyEnq> createState() => _OldNotModifyEnqState();
}

class _OldNotModifyEnqState extends State<OldNotModifyEnq> {
  List<Enqlist> result = [];
  ...
  void getEnqData(userToken, userId, paraPage) async {
    ...
    var response = await http.get(
        Uri.parse(
            '$apiRootUrl/oldnotmodify_enq.php?user_id=$userId&page_no=$page'),
        headers: headersData);
  ...
  void addFavorite(leadId, userToken) async {
    ...
    Navigator.of(context).pushReplacement(MaterialPageRoute(builder: (context) => OldEnquiryPage(tabIndex: 0)));
  }
