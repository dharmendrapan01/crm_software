...
class SiteVisitDone1 extends StatefulWidget {
  const SiteVisitDone1({Key? key}) : super(key: key);

  @override
  State<SiteVisitDone1> createState() => _SiteVisitDone1State();
}

class _SiteVisitDone1State extends State<SiteVisitDone1> {
  List<Enqlist> result = [];
  ...
  //       Uri.parse(
  //           '$apiRootUrl/sitevisit_done_one.php?user_id=$userId&page_no=$page'),
  //       headers: headersData);
  ...
  Future getEnqData(userToken, userId, paraPage) async {
    ...
    var apiUrl = '$apiRootUrl/sitevisit_done_one.php';
  ...
  void addFavorite(leadId, userToken) async {
    ...
    Navigator.of(context).pushReplacement(MaterialPageRoute(builder: (context) => SiteVisitPage(tabIndex: 2)));
  }
