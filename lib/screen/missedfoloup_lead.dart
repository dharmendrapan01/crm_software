...
class MissedFoloupLead extends StatefulWidget {
  const MissedFoloupLead({Key? key}) : super(key: key);

  @override
  State<MissedFoloupLead> createState() => _MissedFoloupLeadState();
}

class _MissedFoloupLeadState extends State<MissedFoloupLead> {
  List<Enqlist> result = [];
  ...
  //       Uri.parse(
  //           '$apiRootUrl/missedfolloup_lead.php?user_id=$userId&page_no=$page'),
  //       headers: headersData);
  ...
  Future getEnqData(userToken, userId, paraPage) async {
    ...
    var apiUrl = '$apiRootUrl/missedfolloup_lead.php';
  ...
  void addFavorite(leadId, userToken) async {
    ...
    Navigator.of(context).pushReplacement(MaterialPageRoute(builder: (context) => MissedFollowup()));
  }
