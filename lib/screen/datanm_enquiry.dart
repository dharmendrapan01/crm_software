...
class DataNotModifyEnq extends StatefulWidget {
  const DataNotModifyEnq({Key? key}) : super(key: key);

  @override
  State<DataNotModifyEnq> createState() => _DataNotModifyEnqState();
}

class _DataNotModifyEnqState extends State<DataNotModifyEnq> {
  List<Enqlist> result = [];
  ...
  //       Uri.parse(
  //           '$apiRootUrl/datanm_enquiry.php?user_id=$userId&page_no=$page'),
  //       headers: headersData);
  ...
  Future getEnqData(userToken, userId, paraPage) async {
    ...
    var apiUrl = '$apiRootUrl/datanm_enquiry.php';
  ...
  void addFavorite(leadId, userToken) async {
    ...
    Navigator.of(context).pushReplacement(MaterialPageRoute(builder: (context) => DataEnquiryPage(tabIndex: 0)));
  }
