...
class ColdLead extends StatefulWidget {
  const ColdLead({Key? key}) : super(key: key);

  @override
  State<ColdLead> createState() => _ColdLeadState();
}

class _ColdLeadState extends State<ColdLead> {
  List<Enqlist> result = [];
  ...
  //       Uri.parse(
  //           '$apiRootUrl/cold_lead.php?user_id=$userId&page_no=$page'),
  //       headers: headersData);
  ...
  Future getEnqData(userToken, userId, paraPage) async {
    ...
    var apiUrl = '$apiRootUrl/cold_lead.php';
  ...
  void addFavorite(leadId, userToken) async {
    ...
    Navigator.of(context).pushReplacement(MaterialPageRoute(builder: (context) => QualifiedPage(tabIndex: 2)));
  }
