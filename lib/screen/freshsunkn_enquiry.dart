...
import 'package:crm_software/freshenquiry_page.dart';
...
class FreshStatusUnknEnq extends StatefulWidget {
  const FreshStatusUnknEnq({Key? key}) : super(key: key);

  @override
  State<FreshStatusUnknEnq> createState() => _FreshStatusUnknEnqState();
}

class _FreshStatusUnknEnqState extends State<FreshStatusUnknEnq> {
  List<Enqlist> result = [];
  ...
  //       Uri.parse(
  //           '$apiRootUrl/freshstatusunkn_enq.php?user_id=$userId&page_no=$page'),
  //       headers: headersData);
  ...
  Future getEnqData(userToken, userId, paraPage) async {
    ...
    var apiUrl = '$apiRootUrl/freshstatusunkn_enq.php';
  ...
  void addFavorite(leadId, userToken) async {
    ...
    Navigator.of(context).pushReplacement(MaterialPageRoute(builder: (context) => FreshEnquiryPage(tabIndex: 1)));
  }
