...
class AllModifiedEnquiry extends StatefulWidget {
  const AllModifiedEnquiry({Key? key}) : super(key: key);

  @override
  State<AllModifiedEnquiry> createState() => _AllModifiedEnquiryState();
}

class _AllModifiedEnquiryState extends State<AllModifiedEnquiry> {
  List<Enqlist> result = [];
  ...
  //       Uri.parse(
  //           '$apiRootUrl/allmodifiedenquiry.php?user_id=$userId&page_no=$page'),
  //       headers: headersData);
  ...
  Future getEnqData(userToken, userId, paraPage) async {
    ...
    var apiUrl = '$apiRootUrl/allmodifiedenquiry.php';
  ...
  void addFavorite(leadId, userToken) async {
    ...
    Navigator.of(context).pushReplacement(MaterialPageRoute(builder: (context) => AllenquiryLead(tabIndex: 1)));
  }
