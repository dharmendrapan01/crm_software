...
import 'package:crm_software/alldeleted_page.dart';
...
class AllDeletedEnquiry extends StatefulWidget {
  const AllDeletedEnquiry({Key? key}) : super(key: key);

  @override
  State<AllDeletedEnquiry> createState() => _AllDeletedEnquiryState();
}

class _AllDeletedEnquiryState extends State<AllDeletedEnquiry> {
  List<Enqlist> result = [];
  ...
  //       Uri.parse(
  //           '$apiRootUrl/alldeleted_enq.php?user_id=$userId&page_no=$page'),
  //       headers: headersData);
  ...
  Future getEnqData(userToken, userId, paraPage) async {
    ...
    var apiUrl = '$apiRootUrl/alldeleted_enq.php';
  ...
  void addFavorite(leadId, userToken) async {
    ...
    Navigator.of(context).pushReplacement(MaterialPageRoute(builder: (context) => AllDeletedPage(tabIndex: 0)));
  }
