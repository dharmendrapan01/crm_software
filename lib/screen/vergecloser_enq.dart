...
import 'package:crm_software/verge_closer_page.dart';
...
class VergeCloserEnq extends StatefulWidget {
  const VergeCloserEnq({Key? key}) : super(key: key);

  @override
  State<VergeCloserEnq> createState() => _VergeCloserEnqState();
}

class _VergeCloserEnqState extends State<VergeCloserEnq> {
  List<Enqlist> result = [];
  ...
  //       Uri.parse(
  //           '$apiRootUrl/vergecloser_lead.php?user_id=$userId&page_no=$page'),
  //       headers: headersData);
  ...
  Future getEnqData(userToken, userId, paraPage) async {
    ...
    var apiUrl = '$apiRootUrl/vergecloser_lead.php';
  ...
  void addFavorite(leadId, userToken) async {
    ...
    Navigator.of(context).pushReplacement(MaterialPageRoute(builder: (context) => VergeCloserPage()));
  }
