...
class AllDataEnquiry extends StatefulWidget {
  const AllDataEnquiry({Key? key}) : super(key: key);

  @override
  State<AllDataEnquiry> createState() => _AllDataEnquiryState();
}

class _AllDataEnquiryState extends State<AllDataEnquiry> {
  List<Enqlist> result = [];
  ...
  //       Uri.parse(
  //           '$apiRootUrl/alldata_enquiry.php?user_id=$userId&page_no=$page'),
  //       headers: headersData);
  ...
  Future getEnqData(userToken, userId, paraPage) async {
    ...
    var apiUrl = '$apiRootUrl/alldata_enquiry.php';
  ...
  void addFavorite(leadId, userToken) async {
    ...
    Navigator.of(context).pushReplacement(MaterialPageRoute(builder: (context) => DataEnquiryPage(tabIndex: 4)));
  }
