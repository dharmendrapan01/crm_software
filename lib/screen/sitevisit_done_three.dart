...
class SiteVisitDoneThree extends StatefulWidget {
  const SiteVisitDoneThree({Key? key}) : super(key: key);

  @override
  State<SiteVisitDoneThree> createState() => _SiteVisitDoneThreeState();
}

class _SiteVisitDoneThreeState extends State<SiteVisitDoneThree> {
  List<Enqlist> result = [];
  ...
  //       Uri.parse(
  //           '$apiRootUrl/svdone_three.php?user_id=$userId&page_no=$page'),
  //       headers: headersData);
  ...
  Future getEnqData(userToken, userId, paraPage) async {
    ...
    var apiUrl = '$apiRootUrl/svdone_three.php';
  ...
  void addFavorite(leadId, userToken) async {
    ...
    Navigator.of(context).pushReplacement(MaterialPageRoute(builder: (context) => SiteVisitPage(tabIndex: 4)));
  }
