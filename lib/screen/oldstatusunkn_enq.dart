...
import 'package:crm_software/old_enquiry_page.dart';
...
class OldStatusUnknEnq extends StatefulWidget {
  const OldStatusUnknEnq({Key? key}) : super(key: key);

  @override
  State<OldStatusUnknEnq> createState() => _OldStatusUnknEnqState();
}

class _OldStatusUnknEnqState extends State<OldStatusUnknEnq> {
  List<Enqlist> result = [];
  ...
  //       Uri.parse(
  //           '$apiRootUrl/oldstatusunkn_enq.php?user_id=$userId&page_no=$page'),
  //       headers: headersData);
  ...
  Future getEnqData(userToken, userId, paraPage) async {
    ...
    var apiUrl = '$apiRootUrl/oldstatusunkn_enq.php';
  ...
  void addFavorite(leadId, userToken) async {
    ...
    Navigator.of(context).pushReplacement(MaterialPageRoute(builder: (context) => OldEnquiryPage(tabIndex: 2)));
  }
