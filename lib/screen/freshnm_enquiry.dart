...
class FreshNotModify extends StatefulWidget {
  const FreshNotModify({Key? key}) : super(key: key);

  @override
  State<FreshNotModify> createState() => _FreshNotModifyState();
}

class _FreshNotModifyState extends State<FreshNotModify> {
  List<Enqlist> result = [];
  ...
  //       Uri.parse(
  //           '$apiRootUrl/freshnm_enq.php?user_id=$userId&page_no=$page'),
  //       headers: headersData);
  ...
  Future getEnqData(userToken, userId, paraPage) async {
    ...
    var apiUrl = '$apiRootUrl/freshnm_enq.php';
  ...
  void addFavorite(leadId, userToken) async {
    ...
    Navigator.of(context).pushReplacement(MaterialPageRoute(builder: (context) => FreshEnquiryPage(tabIndex: 0)));
  }
