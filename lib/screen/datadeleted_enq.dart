...
class DataDeletedEnq extends StatefulWidget {
  const DataDeletedEnq({Key? key}) : super(key: key);

  @override
  State<DataDeletedEnq> createState() => _DataDeletedEnqState();
}

class _DataDeletedEnqState extends State<DataDeletedEnq> {
  List<Enqlist> result = [];
  ...
  //       Uri.parse(
  //           '$apiRootUrl/datadeleted_enq.php?user_id=$userId&page_no=$page'),
  //       headers: headersData);
  ...
  Future getEnqData(userToken, userId, paraPage) async {
    ...
    var apiUrl = '$apiRootUrl/datadeleted_enq.php';
  ...
  void addFavorite(leadId, userToken) async {
    ...
    Navigator.of(context).pushReplacement(MaterialPageRoute(builder: (context) => DataEnquiryPage(tabIndex: 3)));
  }
